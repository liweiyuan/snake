    ...
    public static ElementMatcher.Junction<TypeDescription> buildTypesMatcher(Map<String, String> includeMap, Map<String, String> excludeMap) {
            ...
            if (StringUtils.isNotBlank(namedVal)) {
                ...
                for (String anArr : arr) {
                    if (includeMatcher == null) {
                        includeMatcher = ElementMatchers.named(anArr);
                        continue;
                    }
                    includeMatcher = includeMatcher.or(ElementMatchers.named(anArr));
                }
            }
        ...
        if (excludeMap != null && !excludeMap.isEmpty()) {
            ...
            if (StringUtils.isNotBlank(nameMatchesVal)) {
                String[] arr = nameMatchesVal.split(",");
                for (String anArr : arr) {
                    matcher = matcher.and(ElementMatchers.not(ElementMatchers.<TypeDescription>nameMatches(anArr)));
                }
            }
            if (StringUtils.isNotBlank(hasSuperTypeVal)) {
                String[] arr = hasSuperTypeVal.split(",");
                for (String anArr : arr) {
                    matcher = matcher.and(ElementMatchers.not(ElementMatchers.hasSuperType(ElementMatchers.<TypeDescription>named(anArr))));
                }
            }
            if (StringUtils.isNotBlank(hasAnnotationVal)) {
                String[] arr = hasAnnotationVal.split(",");
                for (String anArr : arr) {
                    matcher = matcher.and(ElementMatchers.not(ElementMatchers.hasAnnotation(ElementMatchers.annotationType(ElementMatchers.<TypeDescription>named(anArr)))));
                }
            }
